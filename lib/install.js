...
const UPDATE = "npm i -g @opsfy/cli";
...
async function install(tool) {
  ...
  if (!validRecipe(recipe)) {
    out(`${tool.name} cannot be installed by this version of opsfy. Update it: ${UPDATE}`);
    return 2;
  }

  if (typeof process.geteuid === "function" && process.geteuid() === 0) {
    out("opsfy never runs as root. Run it as your own user.");
  ...
  if (recipe.kind === "cask") {
    ...
    if (process.env.OPSFY_DRY_RUN === "1") {
      out(`dry-run · would run · brew install --cask ${recipe.cask}`);
      out("dry-run · nothing was installed and nothing was sent");
      return 0;
    }
  ...
  if (process.env.OPSFY_DRY_RUN === "1") {
    out(`dry-run · would clone · ${recipe.repo} into ${recipe.dir}`);
    if (recipe.setup) out(`dry-run · would run · ${recipe.dir}/setup`);
    out("dry-run · nothing was installed and nothing was sent");
    return 0;
  }
